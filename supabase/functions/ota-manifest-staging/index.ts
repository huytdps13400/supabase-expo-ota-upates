...
serve(async (req) => {
  ...
  console.log('[OTA STAGING] Request:', {
    platform,
    runtimeVersion,
  ...
  if (error) {
    console.error('[OTA STAGING] Query failed:', error);
    return new Response('Server Error', { status: 500 });
  }

  console.log('[OTA STAGING] Result:', {
    found: !!update,
    updateId: update?.id,
  });
  ...
  if (assetsError) {
    console.error('[OTA STAGING] Assets query failed:', assetsError);
    return new Response('Server Error', { status: 500 });
  }
